...
// Executa fatorial(n) N_REP vezes e retorna o tempo total em microsegundos
unsigned long medirFatorialUs(int n){
  volatile int r;
  unsigned long t0=micros();
  for(unsigned long i=0;i<N_REP;i++){
    r=fatorial(n);
...
// Imprime tabela do benchmark de fatorial no Serial Monitor
// Para cada N (3, 5, 8): 10 medicoes independentes
void printBenchmarkFatorialSerial(){
  const int ns[3]     = {3, 5, 8};

  Serial.println();
  Serial.println("=========================================");
  Serial.println("  BENCHMARK DE FATORIAL");
  Serial.print  ("  N_REP = "); Serial.print(N_REP); Serial.println(" repeticoes por medicao");
  Serial.println("=========================================");
  Serial.println();
  ...
  for(int g = 0; g < 3; g++){
    ...
    for(int m = 1; m <= 10; m++){
      unsigned long t = medirFatorialUs(n);
      char buf[60];
      snprintf(buf, sizeof(buf), " %2d |    %2d  | %9d | %lu", n, m, res, t);
      Serial.println(buf);
    }

    // Linha separadora entre grupos
    if(g < 2) Serial.println("----|--------|-----------|----------");
  }
...
void setup(){
  ...
  // Roda benchmarks e imprime no Serial antes de iniciar o WiFi
  printBenchmarkSerial();
  printBenchmarkFatorialSerial();
